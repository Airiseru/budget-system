...
export default function RetireeView({ 
    ...
    const canEditCurrentVersion =
        ...
        (
            (data.auth_status === 'draft' && session.user.access_level === 'encode') ||
                    ...
                    <div className="flex justify-end gap-2">
                        <form action={updateAuthStatus}>
                            <button type="submit" className="bg-secondary-foreground text-white px-4 py-2 rounded-md text-sm font-semibold hover:bg-secondary-foreground/80">
                                {session.user.role !== 'dbm' ? 'Submit Form' : 'Finalize Overwrite'}
                            </button>
                        </form>
                    </div>
                </div>
                                ...
                                >
                                    <div className="flex items-center justify-between gap-3">
                                        <span className="text-sm font-bold">
                                            {isOriginal ? `Original (v${versionTab.version})` : `DBM (v${versionTab.version})`}
                                        </span>
