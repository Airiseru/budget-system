...
import { createFormRepository, createRetireeRepository, createSalaryRepository } from "@/src/db/factory";
...
export default async function EditRetireePage({ params }: { params: { id: string } }) {
    ...
    if (retireeData.auth_status !== 'draft' && !(isDbmEvaluator && isPendingDbm)) {
        redirect(`/forms/retirees/${formId}?error=locked`);
    }

    if (!session || (session.user.access_level !== 'encode' && !isDbmEvaluator)) {
    ...
    return (
        <main className="m-4">
            <ButtonGroup className='my-4'>
                <ModeToggle/>
                <ButtonGroup>
                    <BackButton url={`/forms/retirees/${id}`} label="Back"></BackButton>
                </ButtonGroup>
            </ButtonGroup>
            <BP205EntryGrid
                schedule={schedule}
