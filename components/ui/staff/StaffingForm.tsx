...
export default function StaffForm({
    ...
    const handleCompensationChange = (
        ...
        setFormData({ ...formData, positions: updatedPositions });
        clearFieldError(
            `positions.${posIndex}.compensations.${compIndex}.${field}`,
        );
    };
    ...
    async function handleSubmit(e: React.SyntheticEvent<HTMLFormElement>) {
        ...
        if (!result.success) {
            ...
            setError(
                "Validation failed. Please check the highlighted red fields.",
            );
            setIsLoading(false);
        ...
        try {
            ...
            if (response.ok) {
                ...
                router.push(
                    targetId ? `/forms/staff/${targetId}` : "/forms/staff",
                );
            } else {
    ...
    const renderPositionRow = (pos: any, index: number) => {
        ...
        return (
            <>
                <tr
                    key={`pos-${index}`}
                    className="hover:bg-muted/50 border-b border-dotted"
                >
                    ...
                    <td className="p-2 border-r align-top">
                        ...
                        <button
                            ...
                            className="absolute -right-2 -top-2 bg-red-500 text-white rounded-full w-5 h-5 flex items-center justify-center text-[10px] opacity-0 group-hover:opacity-100 transition-opacity"
                        >
                            ✕
                        </button>
                    </td>
                                ...
                                <table className="w-full text-left">
                                    <thead className="bg-muted/50 text-[10px] uppercase font-bold text-muted-500">
                                        <tr>
                                            ...
                                            <th className="px-3 py-2 border-b w-40 text-right">
                                                Amount (PHP)
                                            </th>
                                                                ...
                                                                (name) => {
                                                                    ...
                                                                    return (
                                                                        <option
                                                                            ...
                                                                            disabled={
                                                                                isAlreadySelected
                                                                            } // Disable the option if selected elsewhere
                                                                            className={
                                                                                isAlreadySelected
                ...
                <tr className="">
                    <td colSpan={6} className="px-12 py-1 border-b pb-4 pt-2">
                        <button
                            ...
                            className="bg-primary-foreground p-2 rounded text-[10px] text-primary font-bold uppercase"
                        >
                            + Add Allowance
                        </button>
                    </td>
    ...
    const addCompensation = (posIndex: number) => {
        ...
        if (nextAvailableName) {
            updated[posIndex].compensations.push({
            ...
            alert(
                "All available allowance types have been added for this position.",
            );
        }
    };
    ...
    const removeCompensation = (posIndex: number, compIndex: number) => {
        const updated = [...formData.positions];
        updated[posIndex].compensations = updated[
            posIndex
        ].compensations.filter((_, i) => i !== compIndex);
        setFormData({ ...formData, positions: updated });
    };
    ...
    const TableHeader = () => (
        <thead className="bg-muted/50 border-b text-muted-foreground">
            <tr>
                <th className="p-3 w-1/4 font-semibold text-xs uppercase text-left">
                    Target PAP
                </th>
                <th className="p-3 w-1/4 font-semibold text-xs uppercase text-left">
                    Position / Unit
                </th>
                <th className="p-3 w-24 font-semibold text-xs uppercase text-center">
                    Months
                </th>
                <th className="p-3 w-20 font-semibold text-xs uppercase text-center">
                    Qty
                </th>
                <th className="p-3 w-24 font-semibold text-xs uppercase text-center">
                    SG
    ...
    const renderTierGroups = (tier: number) => {
        ...
        return staffTypes.map((type) => {
            const positions = formData.positions.filter(
                (p) =>
                    p.pap_id &&
    ...
    return (
        <div className="max-w-5xl mx-auto mt-8 px-4 pb-20">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="mt-10">
                    <h3 className="text-sm font-black text-primary-800 border-b-2 border-primary-600 pb-1">
                ...
                <div className="mt-10">
                    <h3 className="text-sm font-black text-emerald-800 border-b-2 border-emerald-600 pb-1">
                ...
                <div className="flex justify-between items-center bg-slate-50 p-4 rounded-lg border">
                    ...
                    <div className="flex gap-3">
                        <button
                            ...
                            className="px-6 py-2 bg-slate-800 text-white rounded-md hover:bg-slate-700 transition-all font-medium"
                        >
                            Save Draft
                        </button>
                        <button
                            ...
                            className="px-6 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-all font-medium"
                        >
                            Submit Form
                        </button>
                    </div>
