...
export const ProposalSchema = z
    ...
    .refine(
        (data) => {
            // Example: If it's an infrastructure project, certain prerequisites might be mandatory
            if (
                data.is_infrastructure &&
                data.cost_by_components.length === 0
            ) {
                return false;
            }
            return true;
        ...
        {
            message: "Infrastructure projects must have at least one component",
            path: ["cost_by_components"],
        },
    );
