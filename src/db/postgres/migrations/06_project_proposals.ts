...
export async function up(db: Kysely<any>): Promise<void> {
    ...
    await db.schema
        ...
        .addColumn('id', 'uuid', (col) => col.primaryKey().defaultTo(sql`gen_random_uuid()`))
        .addColumn('proposal_id', 'uuid', (col) => col.references('project_proposals.id').onDelete('cascade').notNull())
...
export async function down(db: Kysely<any>): Promise<void> {
    ...
    await db.schema.dropTable('local_infrastructure_requirements').ifExists().execute()
    await db.schema.dropTable('local_physical_targets').ifExists().execute()
    await db.schema.dropTable('local_financial_attributions').ifExists().execute()
