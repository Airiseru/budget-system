...
import EntityFormListView, {
    type EntityFormListRow,
} from "@/components/ui/forms/EntityFormListView";
...
export default async function ProposalsPage({
    ...
    try {
        ...
        const filteredRows = allRows.filter((proposal) => {
            if (selectedStatus && proposal.auth_status !== selectedStatus)
                return false;
        ...
        const visibleRows: EntityFormListRow[] = paginate(
            filteredRows,
            safePage,
        ).map((proposal) => ({
            id: proposal.id,
            ...
            subtitle: `${session.user.role === "department" ? "Department" : ""} Priority Rank #${session.user.role === "department" ? proposal.dept_priority_rank : proposal.priority_rank}`,
            ...
            detailLabel: proposal.is_infrastructure
        ...
        return (
            <>
                <EntityFormListView
                    ...
                    phaseNotice={
                        shouldShowBudgetPrepBanner ? (
                            <span>
                                The phase to create new proposals is closed.
                                Please wait for further announcements from DBM.
                            </span>
                        ) : null
                    }
                    typeOptions={[
