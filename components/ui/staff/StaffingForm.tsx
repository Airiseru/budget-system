...
export default function StaffForm({
    ...
    const getBaseSalary = (sg: number, step: number): number =>
        Number(
            schedule?.rates?.find(
                (r) => r.salary_grade === sg && r.step === step,
            )?.amount ?? 0,
        );
    ...
    const updatePosition = (
        index: number,
        updates: Partial<PositionFormInput>,
    ) => {
        setFormData((prev) => {
            ...
            if (
                ...
                merged.total_salary =
                    merged.monthly_base_salary *
                    merged.num_positions *
                    merged.months_employed;
            }
    ...
    const removeRow = (index: number) => {
        setFormData((prev) => {
            ...
            return {
                ...prev,
                positions: updated.length ? updated : [buildDefaultPosition()],
            };
        });
    };
    ...
    async function handleSubmit(e: React.SyntheticEvent<HTMLFormElement>) {
        ...
        const positionsToSubmit = formData.positions.map((p) => {
            ...
            if (p.magna_carta_amount > 0) {
                const mcRule = compensationRules.find((r) =>
                    r.name.toLowerCase().includes("magna carta"),
                );
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                ...
                                <div className="flex items-center gap-3 bg-accent-foreground border border-transparent rounded-lg px-3 py-2">
                                    ...
                                    <div className="relative flex-1">
                                        <span className="absolute left-2 top-[3px] text-slate-500 text-sm">
                                            ₱
                                        </span>
                                        <input
                                            type="number"
                            ...
                            {displayCompensations.length > 0 ? (
                                <div className="bg-card border rounded-lg overflow-hidden shadow-sm mt-3">
                                    <table className="w-full text-left text-sm">
                                        ...
                                        <tbody className="divide-y divide-border">
                                            {displayCompensations.map(
                                                (comp, cIdx) => {
                                                    const rule =
                                                        compensationRules.find(
                                                            (r) =>
                                                                r.id ===
                                ...
                                <p className="text-sm text-muted-foreground italic px-1 mt-3">
                                    No applicable compensation rules for SG{" "}
                                    {pos.salary_grade}.
                                </p>
                            )}
                        </div>
    ...
    const TableHeader = () => (
        <thead className="bg-muted/50 border-b text-muted-foreground">
            <tr>
                <th className="p-3 w-1/4 font-semibold text-sm uppercase text-left">
                    Target PAP
                </th>
                <th className="p-3 w-1/4 font-semibold text-sm uppercase text-left">
                    Position / Unit
                </th>
                <th className="p-3 w-24 font-semibold text-sm uppercase text-center">
                    Months
                </th>
                <th className="p-3 w-20 font-semibold text-sm uppercase text-center">
                    Qty
                </th>
                <th className="p-3 w-24 font-semibold text-sm uppercase text-center">
                    SG
                ...
                <th className="p-3 w-36 font-semibold text-sm uppercase text-center">
                    Total Salary
                </th>
            </tr>
        </thead>
