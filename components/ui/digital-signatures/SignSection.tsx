...
export function SignSection({
    ...
    const shouldShowLatestRejection = !!latestRejection?.remarks && signatories.length === 0
    const shouldShowPastSignatures = shouldShowLatestRejection && pastSignatories.length > 0
        ...
        <div className="border border-border rounded-lg p-6 space-y-4">
            ...
            {shouldShowLatestRejection && (
                <div className="rounded-lg border border-red-300 bg-red-50 p-4 space-y-1">
                    <p className="text-sm font-semibold text-red-900">Latest rejection remarks</p>
                    ...
                    <p className="text-xs text-red-800">
                        ...
                        {' '}on {new Intl.DateTimeFormat('en-PH', {
                            month: 'short',
                            day: 'numeric',
                            year: 'numeric',
                            hour: 'numeric',
                ...
                <details className="rounded-lg border border-border bg-muted/20">
                    ...
                    <div className="space-y-2 border-t border-border px-4 py-3">
                        {pastSignatories.map(sig => (
                            <div key={sig.id} className="rounded-lg border border-border p-3">
                                <p className="text-sm font-medium">
                                    {sig.user_name} ({WORKFLOW_ROLE_LABELS[sig.role] ?? sig.role})
                                </p>
                    ...
                    <div className="flex flex-wrap gap-2">
                        ...
                        {getNextStatus(authStatus, workflow, 'reject') && (
                            <RejectButton
                                ...
                                signatoryRole={signatoryRole}
                                fromAuthStatus={authStatus}
                                toAuthStatus={getNextStatus(authStatus, workflow, 'reject') ?? ''}
                            />
                        )}
                    </div>
                </div>
