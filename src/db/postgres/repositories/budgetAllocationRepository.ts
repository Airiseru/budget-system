...
import {
    NewAllocationWorkflowLog,
    NewBudgetAllocation,
    BudgetAllocation,
    BudgetAllocationUpdate,
} from "@/src/types/line_items";
...
function isValidAllocationStatusTransition(
    ...
    const currentIndex = ALLOCATION_STATUS_ORDER.indexOf(
        currentStatus as (typeof ALLOCATION_STATUS_ORDER)[number],
    );
    const nextIndex = ALLOCATION_STATUS_ORDER.indexOf(
        nextStatus as (typeof ALLOCATION_STATUS_ORDER)[number],
    );
...
export async function createBudgetAllocation(values: NewBudgetAllocation) {
    ...
    try {
        ...
        if (
            isUniqueViolation(error, "budget_allocations_unique_line_item_key")
        ) {
            throw buildDuplicateAllocationError();
        }
...
export async function updateBudgetAllocation(
    ...
    if (values.auth_status) {
        if (
            !isValidAllocationStatusTransition(
                existing.auth_status,
                values.auth_status,
            )
        ) {
            throw new Error(
                `Invalid allocation status transition from ${existing.auth_status} to ${values.auth_status}.`,
            );
        }
    }
    ...
    const nextPapCode =
        values.pap_code !== undefined ? values.pap_code : existing.pap_code;
    const nextFundCode =
        values.fund_code !== undefined ? values.fund_code : existing.fund_code;
    const nextItemCatalogId =
        values.item_catalog_id ?? existing.item_catalog_id;
...
export async function createAllocationWorkflowLog(
    values: NewAllocationWorkflowLog,
) {
    return await db
...
export async function createAllocationWorkflowLogs(
    values: NewAllocationWorkflowLog[],
) {
...
export async function updateAllocationStatusForYear(
    ...
    return await updateAllocationStatusForYearWithExecutor(
        fiscalYear,
        fromStatuses,
        toStatus,
        db,
...
export async function getAllocationSignoffSummary(fiscalYear: number) {
    const summary = await db
        ...
        .select(({ fn }) => [
            ...
            sql<number>`SUM(CASE WHEN valid_from IS NULL OR valid_until IS NULL THEN 1 ELSE 0 END)`.as(
                "missing_validity_count",
            ),
...
export async function getAllocationSignoffSnapshot(
    ...
    const allocations = rows.map((row) => ({
        ...
        amount: Number(
            signoffType === "nep" ? (row.nep_amt ?? 0) : (row.gaa_amt ?? 0),
        ),
        ...(signoffType === "gaa"
            ? {
                  valid_from: row.valid_from
                      ? formatDateOnlyForInput(row.valid_from)
                      : null,
                  valid_until: row.valid_until
                      ? formatDateOnlyForInput(row.valid_until)
                      : null,
              }
            : {}),
    }));
    ...
    return {
        ...
        total_amount: allocations.reduce(
...
export async function countAllocationsMissingValidityByYear(
    fiscalYear: number,
) {
    const result = await db
        ...
        .where("budget_cycle_year", "=", fiscalYear)
        .where(({ or, eb }) =>
            or([eb("valid_from", "is", null), eb("valid_until", "is", null)]),
        )
        .executeTakeFirst();
...
function buildAllocationDashboardBaseQuery(
    filters: AllocationDashboardFilters,
) {
            ...
            (eb) => {
                ...
                if (expenseClass) {
                    itemQuery = itemQuery.where(
                        ...
                        "=",
                        expenseClass as "PS" | "MOOE" | "CO" | "FINEX",
                    );
                }
...
export async function getAllocationDashboardRows(
    filters: AllocationDashboardFilters,
) {
    let query = buildAllocationDashboardBaseQuery(filters)
        ...
        .select([
            ...
            sql<
                string | null
            >`COALESCE(departments.uacs_code, agency_departments.uacs_code, parent_agency_departments.uacs_code)`.as(
                "department_uacs_code",
            ),
            ...
            sql<
                string | null
            >`COALESCE(agencies.uacs_code, parent_agencies.uacs_code)`.as(
                "agency_uacs_code",
            ),
...
export async function getAllocationDashboardAggregates(
    ...
    const result = await buildAllocationDashboardBaseQuery(filters)
        .select(({ fn }) => [
            fn.count<string>("budget_allocations.id").as("count"),
            fn
                .sum<number>("budget_allocations.proposed_amt")
                .as("proposed_total"),
            fn
                .sum<number>("budget_allocations.dbm_rec_amt")
                .as("dbm_rec_total"),
...
export async function getAllocationDashboardTotals(
    filters: Omit<AllocationDashboardFilters, "limit" | "offset">,
) {
